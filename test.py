...
def main():
    ...
    # Get camera list
    device_list_t = cv_device.getDeviceList()
    device_list = []
    assert len(device_list_t) % 2 == 0
    # format camera list
    for i in range(len(device_list_t) // 2):
        dev_name = device_list_t[i*2]
        dev_numb = device_list_t[i*2+1]
        dev_numb = dev_numb.split('#')[2]
        device_list.append(f'{dev_name} ==> {dev_numb}')
    index = 0
